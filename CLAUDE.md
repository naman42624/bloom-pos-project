...
3. **POS/checkout integrity** — core register-guard enforcement is done and live-tested on `feature/identity-roles-pin-login` (`server/utils/register-guard.js`, wired into sale creation, add-payment, refund, `PUT /:id/cancel` — redesigned to block cancelling a sale with an unrefunded balance instead of silently reversing register totals — pickup completion, and both delivery-settlement routes). **Still open, not yet on this branch:** `GET /api/expenses` and `GET /api/purchase-orders` have no `authorize()` guard at all — only `authenticate` — so `role='customer'` (and any other authenticated role) can currently read expense/purchase-order data. This fix was built and tested once already, but only on a now-**stale, superseded** branch, `feature/pos-checkout-integrity` (abandoned mid-session after a branch switch disrupted live testing — its stash `stash@{0}` still has the original WIP; safe to drop once confirmed unneeded, not blocking). Redo this fix fresh on whichever branch is current when picked back up, don't try to cherry-pick from the stale branch. Ordering note: `GET /api/sales` has the same lack of a role guard, deliberately accepted (order detail isn't sensitive the same way) — expenses/purchase-order cost data is genuinely sensitive, this is a real gap.
