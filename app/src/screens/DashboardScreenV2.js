      ...
      <View style={{flex:1,flexDirection:isDesktop&&showPanel?'row':'column'}}>
        <ScrollView style={ms.container} contentContainerStyle={ms.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={P.pink}/>}>
          ...
          {loading?<View style={{alignItems:'center',paddingVertical:50}}><ActivityIndicator color={P.pink} size="large"/><Text style={{color:P.textSec,marginTop:10}}>Loading...</Text></View>:<>
            {/* Global COD Alert */}
            {totalPendingCod > 0 && (
              <TouchableOpacity style={[ms.codAlert, { alignSelf: 'stretch', marginBottom: 12, paddingVertical: 10, paddingHorizontal: 12, backgroundColor: P.amberLight, borderWidth: 1, borderColor: P.amber+'50' }]} onPress={()=>navigation.navigate('More',{screen:'Settlements',initial:false,params:{locationId:registers.find(r=>r.pendingCodTotal>0)?.locationId||activeLocation?.id}})} activeOpacity={0.8}>
