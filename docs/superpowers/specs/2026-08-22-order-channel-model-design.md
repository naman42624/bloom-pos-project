...
| `sales` | ~~Formalize the `status` CHECK constraint~~ — **verified live, already done.** `server/routes/sales.js` runs its own boot-time constraint migration (lines 14-26) that already sets exactly `CHECK(status IN ('pending','confirmed','preparing','ready','completed','cancelled','draft'))`. Confirmed against the actual database via `psql`. No task needed. | |
| `deliveries` | ~~Formalize `cod_status` CHECK~~ — **verified live, not needed.** `cod_status` is plain `text` with no CHECK constraint at all in the live database (the constraint the original survey found was in the dead `schema.sql`, never applied). `'partial'` already writes fine. No task needed. | |
...
4. **Fulfilment** — pickup or delivery toggle.
   - If delivery: **recipient name & phone**, separate from the buyer (wires the existing but under-used `receiver_customer_id`/`receiver_name`/`receiver_phone`/`sender_customer_id`/`sender_name`/`sender_phone`/`sender_message` columns on `sales` consistently — verified live in the database, this is plumbing existing schema, not new columns), address, scheduled date/time, delivery instructions.
